...
<div class="two-col form-first">
  <section class="panel"><h2>有給予定を登録</h2><p class="notice">マネージャーまたは上長への事前確認後に登録してください。この登録による承認処理はありません。</p>
  ...
  <section class="panel"><h2>取得履歴・取消履歴</h2>
    <div class="table-wrap"><table><thead><tr><th>取得日</th><th>区分</th><th>状態</th><th>操作</th></tr></thead><tbody>
      <?php foreach ($entries as $entry): $past = $entry['leave_date'] < date('Y-m-d'); ?><tr><td><?= e($entry['leave_date']) ?></td><td><?= e(['full'=>'1日','am'=>'午前休','pm'=>'午後休'][$entry['leave_type']]) ?></td><td><span class="tag"><?= $entry['status']==='cancelled' ? '取消済み' : ($past ? '取得済み' : '登録済み') ?></span><?php if ($entry['status']==='cancelled'): ?><small><?= e($entry['cancellation_reason']) ?></small><?php endif; ?></td><td>
        <?php if ($entry['status'] !== 'cancelled' && !$past): ?><form method="post" action="<?= e(url('leave/cancel')) ?>" class="inline-form" data-confirm="この有給予定を取り消しますか？"><?= \App\Csrf::field() ?><input type="hidden" name="entry_id" value="<?= (int)$entry['id'] ?>"><input name="reason" placeholder="取消理由" required><button class="danger small">取消</button></form><?php else: ?>—<?php endif; ?>
